...
async function sendPasswordReset(email) {
    ...
    try {
        await auth.sendPasswordResetEmail(email, {
            url: window.location.origin + '/login.html', // Redirect back to login after reset
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // STRICT: Keep theme toggle fixed at top on scroll (mobile fix)
    const themeToggle = document.getElementById('theme-toggle');
    if (themeToggle) {
        // Force fixed position on every scroll
        window.addEventListener('scroll', () => {
            themeToggle.style.position = 'fixed';
            themeToggle.style.top = '20px';
            themeToggle.style.right = '20px';
            themeToggle.style.transform = 'translateZ(0)';
        }, { passive: true });
